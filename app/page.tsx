// Page principale du brandbook — sections ancrées navigables.
// Hero + 8 sections de contenu progressivement enrichies.
  ...
  return (
    <>
      ...
      <BrandSection
        ...
        intro="Une maison gourmande contemporaine, entre pâtisserie fine, boulangerie premium et coffee time accessible. Élégante sans être élitiste."
        bg="ivory"
        bottomLine
      >
          ...
          {["Élégante", "Sobre", "Premium", "Proche", "Gourmande", "Chaleureuse", "Accessible", "Généreuse"].map((kw, i) => (
            <BrandCard key={kw} variant="ghost" animDelay={i * 60} noHover>
              <div className="flex items-baseline gap-3 p-1">
                <span className="font-serif text-xl text-or-champagne/40 font-light shrink-0">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="font-serif text-d-sm font-light text-noir-marquise leading-snug">
                  {kw}
                </span>
              </div>
            </BrandCard>
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { bg: "bg-ivoire-maison",  textColor: "text-noir-marquise", label: "Complet",       desc: "Usage principal",    text: "Maison Marquise" },
            { bg: "bg-noir-marquise",  textColor: "text-ivoire-maison", label: "Inversé",        desc: "Sur fond sombre",   text: "Maison Marquise" },
            { bg: "bg-blanc-marbre",   textColor: "text-noir-marquise", label: "Sans baseline",  desc: "Usage compact",     text: "Maison Marquise" },
            { bg: "bg-or-champagne",   textColor: "text-blanc-marbre",  label: "Monogramme",     desc: "Favicon · Sticker", text: "M" },
          ].map(({ bg, textColor, label, desc, text }, i) => (
            <BrandCard key={label} variant={bg.includes("noir") ? "dark" : bg.includes("or") ? "gold" : "default"} animDelay={i * 80} noHover className="p-0 overflow-hidden">
              <div className={`${bg} flex items-center justify-center min-h-36 md:min-h-44 p-6`}>
                <span className={`font-script ${text === "M" ? "text-5xl" : "text-2xl md:text-3xl"} leading-none ${textColor}`}>
                  {text}
                </span>
              </div>
      ...
      <BrandSection
        ...
        intro="Maison, Gourmand, Signature. Chaque univers a sa palette, son énergie, son contexte d'usage. Ensemble, ils forment le territoire visuel complet de la marque."
        bg="ivory"
        bottomLine
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            <BrandCard key={u.name} variant={u.dark ? "dark" : "default"} animDelay={i * 100} noHover className="p-0 overflow-hidden">
              <div className={`${u.dark ? "bg-noir-marquise" : i === 0 ? "bg-blanc-marbre" : "bg-ivoire-maison"} p-7 space-y-4`}>
                <div className="flex gap-2">
                  {u.colors.map((hex) => (
                    <div key={hex} className="w-5 h-5 rounded-full border border-black/10 shrink-0" style={{ backgroundColor: hex }} />
                  ))}
                </div>
                <p className={`font-serif text-d-sm font-light ${u.dark ? "text-ivoire-maison" : "text-noir-marquise"}`}>
                  {u.name}
                </p>
              </div>
